...
{
    #region PointF
    public class PointUIEditor : UITypeEditor
    {
    ...
    public class PointFConverter : TypeConverter
    {
        ...
        {
            if (sourceType == typeof(string)) return true;//字符串，如："X=00.00,y=00.00" 
        ...
        public override object ConvertFrom(ITypeDescriptorContext context, CultureInfo culture, object value)
        {
            ...
            string item = strValue.Replace("{", "").Replace("}", "").Replace("X", "").Replace("Y", "").Replace("=", "").Trim();
            string[] items = item.Split(new char[] { ',' });

            if (items.Length != 2)
                throw new ArgumentException("Failed to parse Text(" + strValue + ") expected text in the format \"X,Y.\"");

            float x = 0;
            float y = 0;
            float.TryParse(items[0], out x);
            float.TryParse(items[1], out y);
        ...
        {
            //将对象转换为字符串
            if ((destinationType == typeof(string)) && value != null && (PointF)value != null)
            {
                return ((PointF)value).ToString();
            }

            //生成设计时的构造器代码 
            if (destinationType == typeof(InstanceDescriptor) && (PointF)value != null)
            {
                PointF p = (PointF)value;
                ConstructorInfo ctor = typeof(PointF).GetConstructor(new Type[] { typeof(float), typeof(float) });
                return new InstanceDescriptor(ctor, new object[] { p.X, p.Y });
            }
        ...
        {
            float s1 = (float)propertyValues["X"];
            float s2 = (float)propertyValues["Y"];
            return new PointF(s1, s2);//创建实例 
        }
      
        public override PropertyDescriptorCollection GetProperties(ITypeDescriptorContext context, object value, Attribute[] attributes)
        {
            if ((PointF)value != null)
            {
